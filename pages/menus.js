      ...
      <ThemeProvider theme={{ primary: '#03A9F4' }}>
        <div style={{ margin: '60px 16px' }}>
          ...
          <p>Menus can accept MenuSelect and MenuOption SMC components as children,
             or can accept an array of objects as seen here -
          </p>
          <StandAloneMenu open menuItems={menuItems} />
          <h2>Button Menu</h2>
          <p>This component is accessible (navigable by clicks or keyboard events)
            and has an ARIA role defined as menu for screenreaders
          </p>
          <p>Each Option can receive a handleSelect prop that accepts a callback</p>
          <p>
            A menu will open in the direction of available space,{' '}
            defaulting to down and to the right. If there is insufficient{' '}
            room below or to the right of the anchor, the menu will open to{' '}
            where there is room for it. This can be seen in action by resizing the{' '}
            window.
          </p>
